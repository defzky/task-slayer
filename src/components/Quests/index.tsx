...
const Quests: React.FC<QuestsProps> = ({
  ...
  useEffect(() => {
    const loadData = async (result: Record<string, unknown>) => {
      ...
      // First, try to load from DatabaseToki if no local data
      if (!result.quests || (Array.isArray(result.quests) && result.quests.length === 0)) {
        const dbQuests = await getFromTable('quests');
        if (dbQuests && dbQuests.length > 0) {
          loadedQuests = dbQuests;
          console.log('_SYNC_ Loaded quests from DatabaseToki:', dbQuests.length);
        }
      } else {
        loadedQuests = result.quests as Quest[];
      }
        ...
        chrome.storage.sync.get(
          ...
          async (syncRes) => {
            if (Object.keys(syncRes).length > 0) {
              await loadData(syncRes as Record<string, unknown>);
            } else {
              chrome.storage.local.get(
                ['quests', 'dailyStreak', 'lastLoginDate'],
                async (localRes) => {
                  if (Object.keys(localRes).length > 0) {
                    await loadData(localRes as Record<string, unknown>);
  ...
  const saveState = async (
    ...
    if (newQuests) {
      try {
        await saveToTable('quests', newQuests);
        console.log('_SYNC_ Quests saved to DatabaseToki');
      } catch (error) {
        console.error('_SYNC_ Failed to save quests to DatabaseToki:', error);
      }
    }
  };
